...
| `js/views/snapshot.js` | The Snapshot view: one commit rendered as the repository's citymap — a deliberate 2D port of Space Walk's grammar (120-unit squarified world, 0.08 inset streets, plate depth-shading, FNV tile jitter, LOD district labels, the edit/hit/read touch lattice). Reached from the Timeline's By-project commit dots via `space:show-commit` (`nav:false, parent:'time'`); every tile clicks through to the previewer pinned to that commit. |
